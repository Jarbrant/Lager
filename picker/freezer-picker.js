/* ============================================================
AO-LOGIN-02 — Session-guard + PICKER controller | FIL: UI/pages/freezer/picker/freezer-picker.js
Projekt: Fryslager (UI-only / localStorage-first)

Syfte:
- PICKER-sidan ska vara skyddad via session (förnamn + roll + expiry).
...
- Inga gissningar om picker-vyer i HTML (undvik 404); controller hanterar lazy import om ni vill.
...
(function () {
  ...
  // INIT-GUARD
  if (window.__FRZ_PICKER_PAGE_INIT__) return;
  window.__FRZ_PICKER_PAGE_INIT__ = true;

  const SESSION_KEY = "FRZ_SESSION_V1"; // (AO-LOGIN) sessionStorage key
  const REQUIRED_ROLE = "PICKER";
  ...
  const fallback = byId("frzPickerFallback");
  ...
  // -----------------------------
  // (Optional) Lazy import picker views
  // -----------------------------
  async function ensurePickerViewsLoaded() {
    // Vi gissar inte i HTML (undvik 404). Här kan ni styra exakt.
    // Om ni inte har några picker-vyer ännu: returnera bara utan att göra något.
    try {
      // Exempel (om filerna finns):
      // await import("../02-view-picker-withdraw.js");
      // await import("../02-view-picker-history.js");
      return true;
    } catch (e) {
      // Fail-soft: views kanske inte finns ännu -> vi kör ändå, registry kan ha andra views.
      showDebug("Picker-vyer kunde inte lazy-importeras (ok om de ej finns ännu).");
      return false;
    }
  }
  ...
  function initRouterMenu() {
    ...
    if (!viewMenu || !viewRoot || !reg || typeof reg.getViewsForRole !== "function") {
      showDebug("Router/Registry saknas. Kontrollera att 01-view-registry.js är laddad.");
      return;
    }
  ...
  async function pollReady() {
    ...
    if (storeOk && regOk) {
      try {
        // (Optional) lazy import picker views (om de finns)
        await ensurePickerViewsLoaded();

        // Init store as PICKER (role låst här)
    ...
    if (pollTries >= POLL_MAX) {
      ...
      setStatus("LÅST");
      showDebug("Kontrollera script paths + att registry är type=module.");
      return;
    }
