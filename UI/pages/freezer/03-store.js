...
Syfte:
- Public Store API (fasad): init/load/save/getState/getStatus/subscribe
- Users CRUD + RBAC helpers
- Items CRUD + query helpers
- Fail-closed vid korrupt storage, SYSTEM_ADMIN read-only
- 1 storage key: AO-FREEZER_V1 (ingen ny)

NOTERING:
- Denna fil motsvarar "store-lagret" från senaste sanning du klistrade in.
- 02-core.js + 04-contract.js kan brytas ut senare, men här är allt samlat
  för att inte riskera runtimefel medan splitten pågår.

Taggar: GUARD / STORAGE / RBAC / FLOW / DEBUG
============================================================ */
...
(() => {
  ...
  /* -----------------------------
    BLOCK 2 — STATUS + ERROR CODES (AO-02)
  ----------------------------- */
  ...
  /* -----------------------------
    BLOCK 4 — AO-04: ITEM CONTRACT CONSTANTS
  ----------------------------- */
  const FRZ_TEMP_CLASSES = /** @type {const} */ (["FROZEN", "CHILLED", "AMBIENT"]);

  /* -----------------------------
    BLOCK 5 — STORAGE ADAPTER (STORAGE)
  ----------------------------- */
  ...
  function init(opts = {}) {
    ...
    if (!loadRes.ok) {
      // GUARD: fail-closed: lock in memory, DO NOT write anything back
  ...
  /* -----------------------------
    BLOCK 10 — AO-03: RBAC + USERS CRUD (RBAC)
  ----------------------------- */
  ...
  function setUserActive(userId, active) {
    ...
    // GUARD: never leave system with 0 active users
    if (!next) {
    ...
    // FLOW: If we inactivated the currently active user -> fallback
  ...
  /* -----------------------------
    BLOCK 11 — AO-04: ITEMS CRUD (FLOW)
  ----------------------------- */
  ...
  /* -----------------------------
    BLOCK 13 — ITEMS HELPERS (CONTRACT)
  ----------------------------- */
  ...
  /* -----------------------------
    BLOCK 15 — VALIDATION / NORMALIZATION (GUARD)
  ----------------------------- */
